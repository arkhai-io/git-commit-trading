...
async function demonstrateKeyFormats() {
	console.log(
		chalk.blue("🔍 Demonstrating On-Chain Key Registration Data Formats\n"),
	);
	...
	console.log(
		chalk.gray(
			"  uint8 keyType;        // 0=PGPv4, 1=SSHEd25519, 2=SSHSecp256k1, 3=X509",
		),
	);
	console.log(
		chalk.gray(
			"  bytes32 nonceHash;    // Hash of unique nonce for this registration",
		),
	);
	console.log(
		chalk.gray(
			"  bytes sig;            // Cryptographic signature proving key ownership",
		),
	);
	console.log(
		chalk.gray(
			"  string publicKey;     // Key material in specific format per key type",
		),
	);
	...
	if (existsSync(pgpKeyPath)) {
		...
		const pgpClaim = createGitKeyClaim(
			KeyType.PGPv4,
			nonceHash,
			demoSignature,
			pgpKeyMaterial,
	...
	if (existsSync(sshKeyPath)) {
		...
		console.log(
			chalk.gray(
				`  publicKey: ${sshClaim.publicKey} (base64 key material only)`,
			),
		);
	} else {
	...
	console.log(
		chalk.gray("• Storage: Mapping of address => latest GitKeyClaim"),
	);
	console.log(
		chalk.gray("• PGP Format: Full key as base64-encoded binary data"),
	);
	console.log(
		chalk.gray(
			"• SSH Format: Only the base64 key material (no algorithm prefix/comment)",
		),
	);
	console.log(
		chalk.gray("• Verification: Cryptographic signature proves key ownership"),
	);
	console.log(
		chalk.gray(
			"• Retrieval: Server fetches by address and auto-imports for Git verification",
		),
	);
}
