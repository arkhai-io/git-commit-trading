...
async fn test_intent_verification() {
    ...
    let user_intent = env::var("USER_INTENT")
        .unwrap_or_else(|_| "I want to ensure the tests/test_main.py works correctly".to_string());
    let test_repo_url = env::var("TEST_REPO_URL").unwrap_or_else(|_| {
        "https://github.com/VAR-META-Tech/intent-verification-sample-py".to_string()
    });
    let test_commit = env::var("TEST_COMMIT")
        .unwrap_or_else(|_| "b9ce728166ecc8a376986d624531af90aae3167b".to_string());
    ...
    let solution_commit2 = env::var("SOLUTION_COMMIT2")
        .unwrap_or_else(|_| "8b85053596ae139d7eb6437ee74a14cc521bfe0a".to_string());
    ...
    .await
    {
        ...
        Err(e) => {
            panic!("Failed to verify Py sample repository: {}", e);
        }
    }
}
