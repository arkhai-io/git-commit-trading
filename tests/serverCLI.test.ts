...
`;
    ...
    describe("CLI Validation", () => {
        ...
        test("should reject both --past and --listen flags", async () => {
            const result = await runCommandWithEnv(['server', '--past', '--listen'], envFilePath);
        ...
        test("should fail with missing COMMIT_OBLIGATION_ADDRESS in env", async () => {
            const result = await runCommandWithEnv(['server', '--past'], badEnvFilePath);
            ...
            const hasExpectedError = result.stderr.includes('Failed to start server') || 
        ...
        test("should accept valid --past flag", async () => {
            ...
            const result = await runCommandWithEnv(['server', '--past'], envFilePath, 3000); // 3 second timeout
        ...
        test("should accept valid --listen flag", async () => {
            // This test will fail when it tries to connect to blockchain, but it should pass validation
            const result = await runCommandWithEnv(['server', '--listen'], envFilePath, 3000); // 3 second timeout
        ...
        test("should accept custom polling interval", async () => {
            const result = await runCommandWithEnv(['server', '--past', '--polling-interval', '2000'], envFilePath, 3000);
            
            expect(result.stdout).toContain('Polling Interval: 2000ms');
        });

        test("should accept custom timeout", async () => {
            const result = await runCommandWithEnv(['server', '--past', '--timeout', '120000'], envFilePath, 3000);
            
            expect(result.stdout).toContain('Test Timeout: 120000ms');
        });

        test("should reject invalid cleanup flag", async () => {
            const result = await runCommandWithEnv(['server', '--past', '--no-cleanup'], envFilePath, 5000);
...
async function runCommand(args: string[], timeoutMs: number = 10000): Promise<{
    exitCode: number;
    stdout: string;
    stderr: string;
}> {
...
async function runCommandWithEnv(args: string[], envFile: string, timeoutMs: number = 10000): Promise<{
    exitCode: number;
    stdout: string;
    stderr: string;
}> {
