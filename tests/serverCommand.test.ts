	...
	describe("Oracle Functions (Direct Testing)", () => {
		test("should arbitrate past obligations using arbitratePastForEscrow", async () => {
			...
			const arbitrationResult =
				await arbiterClient.oracle.arbitratePastForEscrow({
					escrow: {
					...
					arbitrate: async (obligation: any, demand: any) => {
						...
						const hasValidObligation =
							obligation[0]?.commitHash && obligation[0]?.hosts?.length > 0;
						const hasValidDemand =
							demand[0]?.testsCommitHash && demand[0]?.hosts?.length > 0;
					...
					onAfterArbitrate: async (decision: any) => {
						console.log(
							...
							decision.decision ? "PASSED" : "FAILED",
						);
			...
			console.log(
				`✓ Successfully arbitrated ${arbitrationResult.decisions.length} past obligation(s)`,
			);
		...
		test("should listen for new obligations using listenAndArbitrateForEscrow", async () => {
			...
			try {
				...
				const result = await arbiterClient.oracle.listenAndArbitrateForEscrow({
					...
					arbitrate: async (obligation: any, demand: any) => {
						console.log(
							"  → New obligation detected! Running arbitration logic",
						);
						console.log("  → Obligation:", obligation[0]);
						console.log("  → Demand:", demand[0]);
			...
			try {
				...
				const result = await arbiterClient.oracle.listenAndArbitrateForEscrow({
					...
					arbitrate: async (obligation: any, demand: any) => {
						...
						console.log(
							"  → Validation Result:",
							hasValidObligation && hasValidDemand,
						);
						return hasValidObligation && hasValidDemand;
					},
					onAfterArbitrate: async (decision: any) => {
						console.log(
							"  → Real-time arbitration completed:",
							decision.decision ? "PASSED" : "FAILED",
						);
						console.log("  → Transaction Hash:", decision.hash);
						arbitrationResult = decision;
				...
				console.log(
					"Bob fulfilling escrow (this should trigger the real-time listener)...",
				);
				...
				if (eventDetails) {
					...
					expect(eventDetails.obligation.commitHash).toBe(
						"14acbbd4b4795dc5a8178540e32e1aa9661867ea",
					);
					expect(eventDetails.demand.testsCommitHash).toBe(
						"ab940eceae6702e05b9c03765b7407a054ea84c9",
					);
				}
