	...
	describe("CommitObligation - Contract Interactions", () => {
		test("testDoObligation - Success Case", async () => {
			...
			try {
				...
				const result =
					await aliceClient.commitObligation.doObligation(testCommitData);
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping contract test",
					);
					return; // Skip test
				}
		...
		test("testDoObligation - Different Algorithms", async () => {
			try {
				...
				const sha256Result =
					await aliceClient.commitObligation.doObligation(sha256Data);
				...
				const md5Result =
					await aliceClient.commitObligation.doObligation(md5Data);
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping algorithm test",
					);
					return;
				}
				throw error;
		...
		test("testDoObligation - Multiple Hosts", async () => {
			try {
				...
				const result =
					await aliceClient.commitObligation.doObligation(multiHostData);
				expect(result).toBeDefined();
				expect(typeof result).toBe("object");
				expect(result).toHaveProperty("hash");
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping multi-host test",
					);
					return;
				}
				throw error;
		...
		test("testDoObligation - Empty Hosts Array", async () => {
			try {
				...
				const result =
					await aliceClient.commitObligation.doObligation(emptyHostsData);
				expect(result).toBeDefined();
				expect(typeof result).toBe("object");
				expect(result).toHaveProperty("hash");
				expect(result).toHaveProperty("attested");
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping empty hosts test",
					);
					return;
				}
				throw error;
		...
		test("testDoObligation - Long Hash", async () => {
			try {
				const longHashData: CommitObligationData = {
					commitHash:
						"9b71d224bd62f3785d96d46ad3ea3d73319bfbc2890caadae2dff72519673ca72323abcdef1234567890",
					commitAlgo: CommitAlgo.SHA256,
					hosts: ["longhash.contract.com"],
				};

				const result =
					await aliceClient.commitObligation.doObligation(longHashData);
				expect(result).toBeDefined();
				expect(typeof result).toBe("object");
				expect(result).toHaveProperty("hash");
			} catch (error) {
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping long hash test",
					);
					return;
				}
				throw error;
		...
		test("testDoObligation - Bob Client", async () => {
			...
			try {
				...
				const result =
					await bobClient.commitObligation.doObligation(bobTestData);
				expect(result).toBeDefined();
				expect(typeof result).toBe("object");
				expect(result).toHaveProperty("hash");
				expect(result.hash).toMatch(/^0x[a-fA-F0-9]{64}$/);
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping Bob client test",
					);
					return;
				}
				throw error;
		...
		test("testDoObligation - Verify Attestation Data", async () => {
			try {
				...
				const result =
					await aliceClient.commitObligation.doObligation(testData);
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping attestation verification test",
					);
					return;
				}
				throw error;
		...
		test("testDoObligation - Custom Reference UID", async () => {
			try {
				...
				const result = await aliceClient.commitObligation.doObligation(
					testData,
					customRefUID,
				);
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation custom ref UID test skipped due to contract constraint",
					);
					return;
				}
				throw error;
		...
		test("testGetSchema", async () => {
			try {
				...
				if (
					...
					console.log(
						"⚠️  CommitObligation contract not deployed, skipping schema test",
					);
					return;
				}
				throw error;
	...
	describe("CommitObligation - Pure Functions", () => {
		...
		test("testComplexData", async () => {
			...
			const complexData: CommitObligationData = {
				commitHash:
					"9b71d224bd62f3785d96d46ad3ea3d73319bfbc2890caadae2dff72519673ca72323",
				commitAlgo: CommitAlgo.SHA256,
				hosts: [
