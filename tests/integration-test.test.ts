...
describe("🚀 Complete Integration Test - Real Keys & Commits", () => {
	...
	beforeAll(async () => {
		...
		if (!existsSync(configPath)) {
			throw new Error(
				`Configuration file not found: ${configPath}. Please create and populate integration-test-config.json`,
			);
		}
	...
	test("Complete End-to-End Flow with Real Keys and Commits", async () => {
		...
		if (keyType === "both") {
			...
			console.log(
				chalk.blue(
					"Testing with both PGP and SSH keys with actual Git commits",
				),
			);
			...
			console.log(
				chalk.green("✅ Complete flow verified with both PGP and SSH keys"),
			);
		} else {
			console.log(chalk.blue("\n🎯 Starting Complete Integration Test"));
			console.log(
				chalk.blue(
					`Testing with real ${keyType.toUpperCase()} keys and actual Git commits`,
				),
			);
			...
			console.log(
				chalk.green("✅ Complete flow verified with real keys and commits"),
			);
		}
	}, 120000); // 2 minute timeout for complete flow (both modes might take longer)
});

async function runTestWithKeyType(
	keyType: "pgp" | "ssh",
	config: IntegrationConfig,
	testContext: TestContext,
	commitObligationAddress: `0x${string}`,
) {
	...
	const tempConfig = {
		...config,
		keys: { ...config.keys, preferredKeyType: keyType },
	};
	...
	if (keyType === "pgp") {
		...
		console.log(
			chalk.gray(`   PGP Public Key: ${config.keys.pgp.publicKeyPath}`),
		);
		console.log(
			chalk.gray(`   PGP Private Key: ${config.keys.pgp.privateKeyPath}`),
		);
		console.log(
			chalk.gray(
				`   Has Passphrase: ${config.keys.pgp.passphrase ? "Yes" : "No"}`,
			),
		);
	} else {
		console.log(chalk.gray(`   SSH Key Type: ${config.keys.ssh.keyType}`));
		console.log(
			chalk.gray(`   SSH Public Key: ${config.keys.ssh.publicKeyPath}`),
		);
		console.log(
			chalk.gray(`   SSH Private Key: ${config.keys.ssh.privateKeyPath}`),
		);
	}
	console.log(
		chalk.gray(
			`   Key Material Length: ${keyData.publicKeyMaterial.length} characters`,
		),
	);
	...
	const registrationResult = await registerBobKey(
		globalBobClient,
		keyData,
	...
	if (config.keys.preferredKeyType !== "both") {
		const registeredClaim =
			await globalBobClient.gitIdentityRegistry.getLatestKeyClaim(
				testContext.bob.address,
			);
		...
		console.log(
			chalk.yellow(
				"⚠️ Skipping key verification in 'both' mode (latest key overwrites previous)",
			),
		);
	}
	...
	const escrowResult = await createAliceEscrow(
		globalAliceClient,
		config,
	...
	const fulfillmentResult = await createBobFulfillment(
		globalBobClient,
		config,
	...
	console.log(
		chalk.gray(`   Fulfillment UID: ${fulfillmentResult.attested.uid}`),
	);
	...
	const { unwatch } =
		await globalArbiterClient.oracle.listenAndArbitrateForEscrow({
			escrow: {
	...
	console.log(
		chalk.green(
			`✅ Arbitration completed: ${arbitrationResult ? "PASSED" : "FAILED"}`,
		),
	);
	...
	if (arbitrationResult) {
		...
		console.log(
			chalk.red(
				"✗ Test execution failed - fix the solution or test configuration",
			),
		);

		// Fail the test when arbitration/test execution fails
		throw new Error(
			"Test execution failed during arbitration - solution does not pass the tests",
		);
	}
}
...
function validateConfiguration(config: IntegrationConfig): void {
	...
	if (keyType !== "pgp" && keyType !== "ssh" && keyType !== "both") {
		throw new Error(
			`Invalid preferredKeyType: ${keyType}. Must be 'pgp', 'ssh', or 'both'`,
		);
	}
	...
	for (const path of requiredPaths) {
		...
		const resolvedPath = path.startsWith("/")
			? expandedPath
			: resolve(__dirname, "..", expandedPath);
		if (!existsSync(resolvedPath)) {
			throw new Error(
				`Required ${keyType.toUpperCase()} key file not found: ${path} (resolved to: ${resolvedPath}). Please follow INTEGRATION_TEST_SETUP.md`,
			);
		}
	}

	if (
		config.repositories.alice.testsCommitHash ===
		"YOUR_ALICE_TEST_COMMIT_HASH_HERE"
	) {
		throw new Error(
			"Please update integration-test-config.json with your real commit hashes",
		);
	}

	if (
		config.repositories.bob.ssh.solutionCommitHash ===
		"FILL_WITH_SSH_SIGNED_COMMIT_HASH"
	) {
		throw new Error(
			"Please update integration-test-config.json with your real SSH-signed commit hash",
		);
	}

	if (
		config.repositories.bob.pgp.solutionCommitHash ===
		"FILL_WITH_PGP_SIGNED_COMMIT_HASH"
	) {
		throw new Error(
			"Please update integration-test-config.json with your real PGP-signed commit hash",
		);
	}
}
...
async function loadRealKeys(config: IntegrationConfig) {
	...
	if (keyType === "pgp") {
		...
		const sshKeyType =
			config.keys.ssh.keyType === "ed25519"
				? KeyType.SSHEd25519
...
async function registerBobKey(
	...
	if (keyData.keyType === KeyType.PGPv4) {
		...
		try {
			const pgpPrivateKeyPath =
				config.keys.pgp.privateKeyPath.startsWith("/") ||
				config.keys.pgp.privateKeyPath.startsWith("~")
			...
			signature = await generatePGPSignature(
				pgpPrivateKey,
				message,
				passphrase,
			);
...
async function createAliceEscrow(
	aliceClient: any,
	config: IntegrationConfig,
	oracle: `0x${string}`,
	testContext: TestContext,
...
async function createBobFulfillment(
	bobClient: any,
	config: IntegrationConfig,
	escrowUid: `0x${string}`,
	keyType: "ssh" | "pgp",
	...
	const commitConfig =
		keyType === "ssh"
			? config.repositories.bob.ssh
			: config.repositories.bob.pgp;
...
async function performCompleteArbitration(
	...
	try {
		...
		const senderKeyClaim =
			await arbiterClient.gitIdentityRegistry.getLatestKeyClaim(senderAddress);
		...
		console.log(
			chalk.gray(
				`      Registered key type: ${KeyType[senderKeyClaim.keyType]}`,
			),
		);
		...
		try {
			...
			if (!isValid) {
				...
				console.log(
					chalk.red(
						"   🚨 SECURITY VIOLATION: Solution commit not signed by registered key",
					),
				);
				return false;
			}
			...
			console.log(
				chalk.gray(
					`      Verified against registered key for: ${senderAddress}`,
				),
			);
			...
			console.log(
				chalk.red(
					"   🚨 SECURITY VIOLATION: Could not verify commit signature",
				),
			);
			return false;
		} finally {
...
async function executeRealTests(
	...
	try {
		...
		console.log(
			chalk.gray(`      📁 Test commit: ${demand[0].testsCommitHash}`),
		);
		console.log(
			chalk.gray(`      📁 Solution repo: ${obligation[0].hosts[0]}`),
		);
		console.log(
			chalk.gray(`      📁 Solution commit: ${obligation[0].commitHash}`),
		);
