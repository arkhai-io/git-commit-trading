...
import { makeGitIdentityRegistryClient, type GitIdentityRegistryAddresses } from "../../src/clients/gitIdentityRegistry";
...
export async function setupTest() {
  ...
  const gitIdentityRegistryAddress = await testContext.deployContract(GitIdentityRegistry);
  ...
  const aliceClient = testContext.aliceClient.extend((client: any) => ({
    ...
    gitIdentityRegistry: makeGitIdentityRegistryClient(client.viemClient, gitIdentityRegistryAddresses),
  }));
