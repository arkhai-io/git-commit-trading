...
export class IntegrationTestHelpers {
   ...
   */
  static async checkBlockchainStatus(rpcUrl: string = "http://127.0.0.1:8545"): Promise<BlockchainStatus> {
    try {
  ...
  static async waitForBlockchain(
    ...
    while (Date.now() - startTime < maxWaitMs) {
      const status = await this.checkBlockchainStatus(rpcUrl);
      
      if (status.isRunning) {
        console.log(chalk.green(`✅ Blockchain ready - Block: ${status.blockNumber}, Chain ID: ${status.chainId}`));
        return status;
      }
    ...
    throw new Error(`Blockchain not ready after ${maxWaitMs}ms. Make sure to run: npx hardhat node`);
  }
  ...
  static async startOracleServer(
    ...
    return new Promise((resolve, reject) => {
      ...
      serverProcess.stdout?.on('data', (data) => {
        ...
        if (output.includes('Listening for new obligations') || 
      ...
      serverProcess.stderr?.on('data', (data) => {
        ...
        if (error.includes('EADDRINUSE') || error.includes('Failed to start server')) {
      ...
      serverProcess.on('exit', (code) => {
        if (code !== 0 && !serverReady) {
          reject(new Error(`Oracle server exited with code ${code}. Output: ${serverOutput}`));
        }
      });
      ...
      setTimeout(() => {
        if (!serverReady) {
          serverProcess.kill('SIGTERM');
          reject(new Error(`Oracle server failed to start within 30 seconds. Output: ${serverOutput}`));
        }
      }, 30000);
    });
  }
  ...
  static async validateGitConfig(): Promise<{
    ...
    try {
      ...
      try {
        ...
        if (stdout.trim() === 'ssh') {
          const { stdout: sshKey } = await execAsync('git config user.signingkey');
          if (sshKey.trim()) {
            hasSSHKey = true;
  ...
  static async createTestCommit(
    ...
    try {
      ...
      await execAsync(`echo "Test change at ${timestamp}" >> README.md`, { cwd: repoPath });
   ...
   */
  static async verifyCommitSignature(repoPath: string, commitHash: string): Promise<{
    isValid: boolean;
    ...
    try {
      ...
      try {
        const { stdout } = await execAsync(`git verify-commit ${commitHash}`, { cwd: repoPath });
        return {
          isValid: true,
          signatureType: 'gpg',
          signer: stdout.trim()
        ...
        try {
          const { stdout } = await execAsync(`git log --show-signature -1 ${commitHash}`, { cwd: repoPath });
  ...
  static async printSystemInfo(): Promise<void> {
    ...
    const gitConfig = await this.validateGitConfig();
    console.log(chalk.blue("\n🔧 Git Configuration"));
    console.log(`   ${gitConfig.hasPGPKey ? "✅" : "❌"} PGP Key configured`);
    console.log(`   ${gitConfig.hasSSHKey ? "✅" : "❌"} SSH Key configured`);
    console.log(`   ${gitConfig.gpgSigningEnabled ? "✅" : "❌"} Commit signing enabled`);

    if (gitConfig.errors.length > 0) {
      console.log(chalk.red("\n⚠️ Git Configuration Issues:"));
      gitConfig.errors.forEach(error => console.log(chalk.red(`   • ${error}`)));
    }
  }
}
