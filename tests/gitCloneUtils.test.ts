...
describe("cloneGitRepository Function Tests", () => {
	...
	test("Should clone and checkout specific commit", async () => {
		...
		try {
			...
			const { stdout } = await execAsync(
				`cd "${testDir}" && git rev-parse HEAD`,
			);
			const currentCommit = stdout.trim();
			expect(currentCommit).toBe(commitHash);

			console.log(
				`✅ Repository cloned and checked out to commit: ${currentCommit}`,
			);
		} finally {
			// Clean up
			try {
	...
	test("Should handle checkout errors gracefully", async () => {
		const testDir = path.join(
			tmpdir(),
			"clone-checkout-error-unit-test-" + Date.now(),
		);
	...
	test("Should work with repository URLs without .git suffix", async () => {
		const testDir = path.join(
			tmpdir(),
			"clone-no-git-suffix-test-" + Date.now(),
		);
		...
		try {
			...
			console.log(
				"✅ Repository cloned successfully from URL without .git suffix",
			);
		} finally {
			// Clean up
			try {
