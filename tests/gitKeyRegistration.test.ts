...
`;
		...
		const testPublicKey =
			"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIL/FZ5QqNBE4xp4GsH2SZqzqeFeHt6fLHfn4FcBUU2LN test@example.com";
...
-----END OPENSSH PRIVATE KEY-----`;
	...
	const runCLICommand = (
		...
		return new Promise((resolve) => {
			...
			const child = spawn(
				"bun",
				["run", path.join(__dirname, "../src/cli/git-escrows.ts"), ...args],
				{
					cwd,
	...
	describe("register-key Command", () => {
		test("should show help for register-key command", async () => {
			...
			expect(result.stdout).toContain(
				"Register your Git cryptographic key (SSH, PGP, or X509) to your Ethereum",
			);
		...
		test("should auto-detect SSH key files", async () => {
			...
			expect(
				result.stderr.includes("SSH key") || result.stdout.includes("SSH key"),
			).toBeTruthy();
		});
		...
		test("should handle missing SSH key files gracefully", async () => {
			...
			const hasExpectedError =
				result.stderr.includes("Key file not found") ||
	...
	describe("check-key Command", () => {
		test("should show help for check-key command", async () => {
			...
			expect(result.exitCode).toBe(0);
			expect(result.stdout).toContain(
				"Check if your Git SSH key is registered",
			);
...
NETWORK=anvil`;
	...
	describe("Enhanced server Command", () => {
		...
		test("should reject conflicting modes", async () => {
			...
			expect(result.exitCode).toBe(1);
			expect(result.stderr).toContain(
				"Cannot use both --past and --listen options",
			);
		});
	});
...
ADDRESS=not-an-address`;
			...
			expect(
				result.stderr.includes(
					...
					result.stderr.includes("ADDRESS must be a valid Ethereum address"),
			).toBeTruthy();
	...
	describe("SSH Key Detection and Validation", () => {
		test("should handle different SSH key types", async () => {
			...
			const keyTypes = [
				...
				},
				{
					name: "rsa",
					content:
						"ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQC7vI... test@example.com",
				},
				{
					name: "ecdsa",
					content:
						"ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYA... test@example.com",
				},
			];
		...
		test("should reject invalid SSH key format", async () => {
			...
			const hasKeyValidationError =
				result.stderr.includes("Invalid SSH public key format") ||
			...
			const hasContractConfigError =
				result.stderr.includes('Chain "Foundry" is not supported') ||
				result.stderr.includes("no custom contract addresses");

			const hasEnvValidationError =
				result.stderr.includes("Missing required environment variables") ||
				result.stderr.includes("GIT_IDENTITY_REGISTRY_ADDRESS");

			// Any of these error types is acceptable for this test
			const hasExpectedError =
				hasKeyValidationError ||
				hasContractConfigError ||
				hasEnvValidationError;
			expect(hasExpectedError).toBeTruthy();
		});
	});
});
...
describe("GitIdentityRegistry Contract Integration", () => {
	...
	test("should create valid GitKeyClaim structure", () => {
		const {
			KeyType,
			createGitKeyClaim,
		} = require("../src/clients/gitIdentityRegistry");
...
describe("Multi-Key Type Verification", () => {
	...
	test("SSH Ed25519 verification", async () => {
		...
		const gitKeyClaim = {
			...
			fingerprint:
				"0xe49369c35cbe4d28532112f23af38ee79bea8b324b49677a229e6b6e126a6a1d" as `0x${string}`,
			nonceHash:
	...
	test("X509 verification (fallback to GitHub)", async () => {
		...
		const gitKeyClaim = {
			...
			fingerprint:
				"0x567890abcdef1234567890abcdef1234567890ab" as `0x${string}`,
			nonceHash:
				"0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef" as `0x${string}`,
			sig: "0xfedcba0987654321fedcba0987654321fedcba0987654321fedcba0987654321" as `0x${string}`,
