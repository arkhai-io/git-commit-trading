...
describe("PGP Key Claim Signature", () => {
	test("should generate and verify PGP signature for GitKeyClaim", async () => {
		console.log(
			"🧪 Testing PGP GitKeyClaim signature generation and verification",
		);
		...
		const signatureHex = await generatePGPSignature(
			privateKeyArmored,
			signingMessage,
			"test-passphrase",
		);
		...
		console.log(
			`  Decoded signature format: ${armoredSignature.substring(0, 50)}...`,
		);
	...
	test("should reject invalid PGP signature", async () => {
		...
		const isValid = await verifyGitKeyClaimSignature(
			gitKeyClaim,
			differentAddress,
		);
	...
	test("should verify PGP signature with mixed-case address (backward compatibility)", async () => {
		...
		console.log(
			"  This tests backward compatibility for keys registered before address normalization",
		);
		...
		const nonceHex =
			"0000019a71c719f4b67c110f0938cb771204117b7120a7a3bf2c54f426f45550";
		...
		expect(isValid).toBe(true);
		console.log(
			"  ✅ Mixed-case signature verified with lowercase address (backward compatible)!",
		);
	}, 30000);
});
