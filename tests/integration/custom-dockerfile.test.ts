...
const EXAMPLES_DIR = path.resolve(__dirname, "../../examples/custom-dockerfile");
...
describe("custom-dockerfile framework integration", () => {
	test("detects custom dockerfile and runs tests successfully", async () => {
		if (!containerAvailable) {
		...
		expect(result.frameworkUsed).toBe("custom");
