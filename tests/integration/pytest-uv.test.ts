...
const EXAMPLES_DIR = path.resolve(__dirname, "../../examples/pytest-uv");
...
describe("pytest-uv framework integration", () => {
	test("detects pytest-uv framework and runs tests successfully", async () => {
		if (!containerAvailable) {
		...
		expect(result.frameworkUsed).toBe("pytest-uv");
