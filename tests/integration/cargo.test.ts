...
const EXAMPLES_DIR = path.resolve(__dirname, "../../examples/cargo");
...
describe("cargo framework integration", () => {
	test("detects cargo framework and runs tests successfully", async () => {
		if (!containerAvailable) {
		...
		const result = await runTests(testsDir, sourceDir, {
			timeout: 180000, // 3 minutes (cargo can be slow)
		});

		expect(result.frameworkUsed).toBe("cargo");
