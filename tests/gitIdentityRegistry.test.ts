...
describe("GitIdentityRegistry Client", () => {
    ...
    it("should create a git key claim correctly", () => {
        ...
        expect(claim.fingerprint).toBe("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef");
    ...
    it("should have correct client methods", () => {
        ...
        expect(typeof aliceClient.gitIdentityRegistry.fingerprintToAddress).toBe("function");
    ...
    describe("GitIdentityRegistry - Contract Interactions", () => {
        it("should claim a Git key successfully", async () => {
            try {
                ...
                if ((error as Error)?.message?.includes("no code at address") ||
                    ...
                    console.log("⚠️  GitIdentityRegistry contract not deployed, skipping contract test");
                    return;
                }
                throw error;
            }
        ...
        it("should get claimant for a fingerprint", async () => {
            try {
                const fingerprint = "0x1111111111111111111111111111111111111111111111111111111111111111" as `0x${string}`;
                ...
                const claimant = await aliceClient.gitIdentityRegistry.getClaimant(fingerprint);
        ...
        it("should map fingerprint to address", async () => {
            try {
                const fingerprint = "0x9876543210fedcba9876543210fedcba9876543210fedcba9876543210fedcba" as `0x${string}`;
                ...
                const mappedAddress = await bobClient.gitIdentityRegistry.fingerprintToAddress(fingerprint);
                ...
                if ((error as Error)?.message?.includes("no code at address") ||
                    (error as Error)?.message?.includes("CALL_EXCEPTION") ||
                    (error as Error)?.message?.includes("Already claimed")) {
                    console.log("⚠️  GitIdentityRegistry fingerprint mapping test skipped");
                    return;
                }
                throw error;
            }
        ...
        it("should prevent claiming already claimed keys", async () => {
            try {
                const fingerprint = "0x5555555555555555555555555555555555555555555555555555555555555555" as `0x${string}`;
