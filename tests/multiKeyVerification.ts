...
import { detectKeyTypeFromContent, validateKeyForGitSigning, generateKeyFingerprint } from "../src/utils/keyUtils";

describe("Enhanced Multi-Key Verification System", () => {
    
    describe("Key Type Detection", () => {
        test("should detect SSH Ed25519 keys", () => {
            const sshKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDpOOgAtLLU/X72Fku+nmmAhgeXGDzfF7sdYRiyxS7Qt user@example.com";
            expect(detectKeyTypeFromContent(sshKey)).toBe(KeyType.SSHEd25519);
        });

        test("should detect SSH RSA keys", () => {
            const sshKey = "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQC7vbqajDjI... user@example.com";
...
-----END CERTIFICATE-----`;
    ...
    describe("Key Validation", () => {
        test("should validate SSH keys for Git signing", async () => {
            const sshKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDpOOgAtLLU/X72Fku+nmmAhgeXGDzfF7sdYRiyxS7Qt";
            const validation = await validateKeyForGitSigning(KeyType.SSHEd25519, sshKey);
        ...
        test("should detect invalid key material", async () => {
            ...
            const validation = await validateKeyForGitSigning(KeyType.SSHEd25519, invalidKey);
        ...
        test("should validate PGP keys with warnings", async () => {
            ...
            const validation = await validateKeyForGitSigning(KeyType.PGPv4, mockPgpKey);
    ...
    describe("Signature Verification Integration", () => {
        ...
        test("should reject unverified GitHub signatures", async () => {
            ...
            const result = await verifyCommitSignature(unverifiedGitMetadata, gitKeyClaim);
...
-----END PGP PUBLIC KEY BLOCK-----`;
        ...
        test("should handle GitHub-style SSH keys", () => {
            ...
            const githubSshKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDpOOgAtLLU/X72Fku+nmmAhgeXGDzfF7sdYRiyxS7Qt";
...
-----END CERTIFICATE-----`;
    ...
    describe("Security Features", () => {
        ...
        test("should show supported key types and algorithms", () => {
            const supportedTypes = [
                { type: KeyType.SSHEd25519, name: "SSH Ed25519", status: "✅ Full Support" },
                { type: KeyType.SSHSecp256k1, name: "SSH RSA/ECDSA", status: "✅ Full Support" },
                { type: KeyType.PGPv4, name: "PGP v4", status: "⚠️ GitHub Fallback" },
                { type: KeyType.X509, name: "X.509 Certificates", status: "⚠️ GitHub Fallback" }
            ];
        ...
        test("should demonstrate fingerprint generation", async () => {
            const sshKey = "AAAAC3NzaC1lZDI1NTE5AAAAIDpOOgAtLLU/X72Fku+nmmAhgeXGDzfF7sdYRiyxS7Qt";
            const fingerprint = await generateKeyFingerprint(KeyType.SSHEd25519, sshKey);
            
            expect(fingerprint).toBeDefined();
            expect(typeof fingerprint).toBe('string');
            expect(fingerprint.length).toBeGreaterThan(10);
            
            console.log(`🔍 SSH Ed25519 Fingerprint: ${fingerprint.substring(0, 16)}...`);
        });
    });
    ...
    describe("Git Integration Scenarios", () => {
        test("should simulate real-world Git commit verification", async () => {
            ...
            const verificationResult = await verifyCommitSignature(mockGitMetadata, gitKeyClaim);
