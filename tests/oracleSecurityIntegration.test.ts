...
describe("Oracle CommitObligation Integration Tests", () => {
	...
	describe("Security Flow", () => {
		test("Oracle Integration - Git Key Registration Before Fulfillment", async () => {
			console.log(
				"🔐 Testing Security Flow: Registration → Fulfillment → Verification",
			);
			...
			const sshPublicKey =
				"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDpOOgAtLLU/X72Fku+nmmAhgeXGDzfF7sdYRiyxS7Qt bob@enhanced.com";
			...
			const registeredClaim =
				await bobClient.gitIdentityRegistry.getLatestKeyClaim(bob);
					...
					arbitrate: async (obligation: any, demand: any) => {
						...
						try {
							...
							if (
								...
								senderKeyClaim.publicKey.trim() === ""
							) {
								console.log(
									"❌ SECURITY VIOLATION: No registered Git key found for sender",
								);
								console.log(
									"   This is the security gap that the system prevents!",
								);
								return false;
							}
							...
							const isValidClaim = isTestSignature
								? true
								: await verifyGitKeyClaimSignature(
										senderKeyClaim,
										senderAddress,
									);
							...
							if (!isValidClaim) {
								console.log(
									"❌ SECURITY VIOLATION: GitKeyClaim signature is invalid",
								);
								return false;
							}
							console.log(
								"✅ GitKeyClaim signature validated" +
									(isTestSignature ? " (test mode)" : ""),
							);
							...
							console.log(
								"📝 Checking if commit was signed by registered SSH key...",
							);
							...
							if (!isSignedBySender) {
								console.log(
									"❌ SECURITY VIOLATION: Commit was not signed by registered key",
								);
								return false;
							}
							console.log(
								"✅ Commit signature verification passed (test mode)",
							);
						} catch (error) {
						...
						try {
							...
							console.log(
								`✅ Test execution result: ${mockTestResult ? "PASSED" : "FAILED"}`,
							);
		...
		test("Security Validation - Reject Fulfillment Without Registered Key", async () => {
			...
			const mockArbitrationWithoutKey = async (
				obligation: any,
				demand: any,
			) => {
				const senderAddress = obligation[0].sender;
			...
			console.log(
				"✅ Security validation passed - unregistered users rejected",
			);
		});
			...
			const supportedKeyTypes = [
				{
					...
					name: "Ed25519",
					sample: "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5...",
				},
				{
					type: KeyType.SSHSecp256k1,
					name: "RSA/Secp256k1",
					sample: "ssh-rsa AAAAB3NzaC1yc2EAAAADAQAB...",
				},
				{
					type: KeyType.PGPv4,
					name: "PGP",
