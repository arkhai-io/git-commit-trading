...
export async function verifyCommitSignature(
	...
	console.warn(
		"⚠️ verifyCommitSignature is deprecated. Use git native verification instead.",
	);
...
async function verifyPGPSignature(
	...
	try {
		...
		try {
			...
			if (
				gitKeyClaim.publicKey.includes("-----BEGIN PGP PUBLIC KEY BLOCK-----")
			) {
				// Full armored key
				publicKey = await openpgp.readKey({
					armoredKey: gitKeyClaim.publicKey,
				});
			} else {
		...
		try {
			signature = await openpgp.readSignature({
				armoredSignature: gitMetadata.signature,
			});
 ...
 */
function verifySSHEd25519Signature(
	gitMetadata: any,
	gitKeyClaim: GitKeyClaim,
): boolean {
 ...
 */
function verifySSHSecp256k1Signature(
	gitMetadata: any,
	gitKeyClaim: GitKeyClaim,
): boolean {
	...
	try {
		...
		console.log(
			"  ⚠️ SSH Secp256k1 verification not fully implemented - trusting GitHub",
		);
		return gitMetadata.verified;
	} catch (error) {
...
async function verifyX509Signature(
	...
	try {
		...
		if (
			gitMetadata.signature.includes("-----BEGIN PKCS7-----") ||
			...
			try {
				...
				console.log(
					"    ⚠️ S/MIME signature verification not fully implemented - trusting GitHub",
				);
				return gitMetadata.verified;
			} catch (error) {
		...
		// Check for raw certificate signature format
			...
			try {
				...
				console.log(
					"    ⚠️ Raw certificate signature verification not fully implemented - trusting GitHub",
				);
				return gitMetadata.verified;
			} catch (error) {
				console.log(
					"    ❌ Error processing raw certificate signature:",
					error,
				);
				return gitMetadata.verified;
			}
...
export function verifySSHSignature(
  ...
  Key Type: ${keyType}`);
			...
			console.log(
				`  [${traceId}] Message hex dump: ${messageBuffer.toString("hex")}`,
			);
			console.log(
				`  [${traceId}] Message byte analysis: ${JSON.stringify(message.split("").map((c) => c + "(" + c.charCodeAt(0) + ")"))}`,
			);
		}
		...
		if (isDebugMode) {
			console.log(
				`  [${traceId}] Signature buffer length: ${signatureBuffer.length} bytes`,
			);
		}
		...
		if (keyType === "ed25519" && signatureBuffer.length === 64) {
			...
			try {
				...
				if (isDebugMode) {
					console.log(
						`  [${traceId}] Parsed key type: ${key.type}, key size: ${key.size}`,
					);
				}
				...
				if (isDebugMode) {
					console.log(
						`  [${traceId}] Updated verifier with message (${messageBuffer.length} bytes)`,
					);
				}
				...
				if (isDebugMode) {
					console.log(
						`  [${traceId}] Parsing signature as ed25519 raw format...`,
					);
				}
				...
				if (isDebugMode) {
					console.log(
						`  [${traceId}] Parsed signature object: type=${signatureObj.type}, hashAlgorithm=${signatureObj.hashAlgorithm}`,
					);
				}
				...
				if (isDebugMode) {
					console.log(
						`  [${traceId}] ✅ Signature verification result: ${result}`,
					);
				}
...
async function verifyPGPKeyClaimSignature(
	...
	try {
		...
		if (isDebugMode) {
			console.log(
				"  PGP signature (armored):",
				signatureArmored.substring(0, 100) + "...",
			);
		}
...
export async function verifyGitKeyClaimSignature(
	...
	try {
		...
		console.log(
			`  Signature from contract: ${signature.slice(0, 20)}... (length: ${signature.length})`,
		);
	...
	try {
		...
		if (!privateKey.isDecrypted()) {
			if (passphrase) {
				...
				console.log(
					"⚠️  Private key is encrypted but no passphrase provided, generating test signature",
				);
