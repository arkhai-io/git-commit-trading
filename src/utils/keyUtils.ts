 ...
 */
export async function importSSHKeyToServer(publicKey: string, identity: string): Promise<boolean> {
    try {
 ...
 * @returns Promise<boolean> - Success status
 */
export async function importGPGKeyToServer(publicKey: string, identity: string): Promise<boolean> {
    try {
        const fs = await import('fs/promises');
        const path = await import('path');
 ...
 * @returns Promise<boolean> - Success status
 */
export async function removeSSHKeyFromServer(identity: string): Promise<boolean> {
    try {
        const fs = await import('fs/promises');
        const path = await import('path');
        ...
        try {
            ...
            const filteredLines = lines.filter(line => !line.startsWith(`${identity} `));
            
            await fs.writeFile(allowedSignersFile, filteredLines.join('\n'));
            console.log(`✅ SSH key removed from allowed_signers for identity: ${identity}`);
            return true;
 ...
 * @returns Promise<boolean> - Success status
 */
export async function removeGPGKeyFromServer(identity: string, keyFingerprint?: string): Promise<boolean> {
    try {
        const { exec } = await import('child_process');
        const { promisify } = await import('util');
        const execAsync = promisify(exec);
        ...
        if (keyFingerprint) {
            ...
            const { stdout, stderr } = await execAsync(`gpg --delete-keys --batch --yes ${keyFingerprint}`, {
                timeout: 30000,
            });
            ...
            console.warn(`⚠️ Removing GPG keys by identity ${identity} - this may remove multiple keys`);
            
            try {
                const listResult = await execAsync(`gpg --list-keys --with-colons | grep "${identity}"`, { timeout: 10000 });
                if (listResult.stdout) {
                    console.log('Found GPG keys for identity, manual removal may be needed');
                }
            } catch (listError) {
 ...
 * @returns Promise<boolean> - True if key exists
 */
export async function isGPGKeyImported(keyFingerprint: string): Promise<boolean> {
    try {
        const { exec } = await import('child_process');
        const { promisify } = await import('util');
        const execAsync = promisify(exec);
...
export async function initializeServerGitEnvironment(): Promise<boolean> {
    try {
        ...
        try {
            ...
            await fs.writeFile(allowedSignersFile, '# Git SSH signature verification\n# Format: identity ssh-keytype keydata\n');
            await fs.chmod(allowedSignersFile, 0o600);
        }
        ...
        for (const [key, value] of gitConfigs) {
            try {
                await execAsync(`git config --global ${key} ${value}`, { timeout: 5000 });
            } catch (error) {
 ...
 */
export async function getServerGitCapabilities(): Promise<{ssh: boolean, gpg: boolean, git: boolean}> {
    const { exec } = await import('child_process');
    const { promisify } = await import('util');
    const execAsync = promisify(exec);
