...
export async function verifyRepo(
  ...
  try {
    ...
    const signatureInfo = await getCommitSignature(repoDir, commitHash, timeoutMs);
    ...
    if (signatureInfo.signatureType !== expectedSigType) {
      console.log(`❌ Signature type mismatch: expected ${expectedSigType}, got ${signatureInfo.signatureType}`);
      return false;
    }

    // Configure git for verification and run git verify-commit
    await configureGitForKey(repoDir, keyType, publicKey);
    const verifyResult = await runGitVerifyCommit(repoDir, commitHash, timeoutMs);
...
async function configureGitForKey(
  ...
  try {
    if (keyType === KeyType.SSHEd25519 || keyType === KeyType.SSHSecp256k1) {
      ...
      const keyPrefix = keyType === KeyType.SSHEd25519 ? 'ssh-ed25519' : 'ecdsa-sha2-nistp256';
...
async function matchGpgKey(
  ...
  try {
    ...
    if (gitOutput) {
      ...
      if (gitFingerprint) {
        const cleanGitFingerprint = gitFingerprint.replace(/\s/g, '').toUpperCase();
        ...
        if (cleanGitFingerprint.endsWith(cleanStoredFingerprint.slice(-16))) return true;
      }
    }
...
function matchSshKey(
  ...
  if (calculatedFingerprint && fingerprint && calculatedFingerprint === fingerprint) {
    return true;
  }
