...
function detectKeyType(keyContent: string): KeyType {
	try {
		...
		if (keyContent.includes("ssh-ed25519")) {
			...
			throw new Error(
				"Unsupported key type. Supported: SSH (ed25519, rsa, ecdsa), PGP, X509",
			);
		}
	}
}
 ...
 */
function readCryptographicKey(options: RegisterKeyOptions): {
	content: string;
	type: KeyType;
} {
	let keyPath: string;
	...
	if (options.pgpKeyFile) {
		...
		if (!keyPath) {
			throw new Error(
				"No key found. Use --path, --pgp-key-file, or --x509-cert-file to specify the key file location",
			);
		}
	}
...
function readSSHPrivateKey(
	...
	if (options.privateKeyFile) {
		...
		if (!privateKeyPath) {
			throw new Error(
				"No SSH private key found for signing. Use --private-key-file to specify the private key location.",
			);
		}
	}
...
function readPGPPrivateKey(options: RegisterKeyOptions): string {
	...
	if (options.pgpPrivateKeyFile) {
		...
		if (!privateKeyPath) {
			throw new Error(
				"No PGP private key found for signing. Use --pgp-private-key-file to specify the private key location.",
			);
		}
	}
	...
	if (!privateKeyContent.includes("-----BEGIN PGP PRIVATE KEY BLOCK-----")) {
		throw new Error(
			"Invalid PGP private key format. Expected ASCII armored format.",
		);
	}
...
export async function registerKeyCommand(options: RegisterKeyOptions) {
	try {
		...
		const { content: keyContent, type: keyType } =
			readCryptographicKey(options);
		console.log(
			chalk.gray(
				`Found ${getKeyTypeName(keyType)} key: ${keyContent.substring(0, 50)}...`,
			),
		);
		...
		console.log(
			chalk.gray(
				`Key material prepared for storage (${keyMaterial.length} characters)`,
			),
		);
		...
		if (keyType === KeyType.SSHEd25519 || keyType === KeyType.SSHSecp256k1) {
			...
			try {
				...
				const pgpSignatureHex = await generatePGPSignature(
					privateKeyContent,
					signingMessage,
					passphrase,
				);
			...
			console.log(
				chalk.yellow("⚠️ X509 signature generation not yet implemented"),
			);
			console.log(chalk.yellow("   Using placeholder signature for testing"));
			signature =
				"0x" +
				Buffer.from(`placeholder_sig_${keyType}_${Date.now()}`)
					.toString("hex")
					.padStart(128, "0");
		}

		console.log(
			chalk.gray(`Generated signature: ${signature.substring(0, 32)}...`),
		);

		// Create the Git key claim
		const gitKeyClaim = createGitKeyClaim(
			keyType,
			nonceHash,
			signature,
			keyMaterial,
		...
		if (!options.skipServerImport) {
			...
			const keyForServerImport =
				keyType === KeyType.PGPv4 ? keyContent : gitKeyClaim.publicKey;
			...
			if (serverImportResult) {
				...
				console.log(
					chalk.yellow(
						"⚠️ Key import to server failed (verification may fail without proper key import)",
					),
				);
			}
		} else {
		...
		console.log(
			chalk.yellow(
				`  1. Your Git commits signed with this ${getKeyTypeName(keyType)} key are now linked to your Ethereum address`,
			),
		);
		console.log(
			chalk.yellow(
				"  2. You can now fulfill escrows and prove authorship of commits",
			),
		);
		console.log(
			chalk.yellow(
				"  3. Use this same key to sign Git commits for escrow fulfillments",
			),
		);
		console.log(
			chalk.yellow(
				"  4. The server can now verify your commit signatures locally",
			),
		);
	} catch (error) {
		...
		console.log(
			chalk.yellow(
				"  1. For SSH keys: Ensure you have SSH keys generated: ssh-keygen -t ed25519",
			),
		);
		console.log(
			chalk.yellow(
				"  2. For PGP keys: Use --pgp-key-file to specify PGP public key file",
			),
		);
		console.log(
			chalk.yellow(
				"  3. For X509 certificates: Use --x509-cert-file to specify certificate file",
			),
		);
		console.log(
			chalk.yellow(
				"  4. Check that your .env file contains GIT_IDENTITY_REGISTRY_ADDRESS",
			),
		);
		console.log(
			chalk.yellow(
				"  5. Verify your private key has sufficient gas for the transaction",
			),
		);
		console.log(
			chalk.yellow(
				"  6. Use --skip-server-import if server key import is not needed",
			),
		);
 ...
 */
async function importKeyToServer(
	keyType: KeyType,
	publicKey: string,
	address: string,
): Promise<boolean> {
	try {
		switch (keyType) {
			...
			case KeyType.X509:
				console.log(
					chalk.yellow("   X509 certificate import not yet implemented"),
				);
				return false;
