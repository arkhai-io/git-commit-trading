...
export async function fulfillCommand(options: FulfillOptions) {
	try {
		...
		if (
			!options.escrowUid ||
			!options.solutionRepo ||
			!options.solutionCommit
		) {
			throw new Error(
				"Missing required options: --escrow-uid, --solution-repo, --solution-commit",
			);
		}
		...
		const solutionAlgo =
			commitAlgoMap[options.solutionAlgo?.toLowerCase() || "sha1"];
		if (solutionAlgo === undefined) {
			throw new Error(
				`Invalid commit algorithm: ${options.solutionAlgo}. Use: sha1, sha256, or md5`,
			);
		}
		...
		if (options.verifyKey !== false && hasGitIdentityRegistry) {
			...
			try {
				...
				if (
					...
					console.log(
						chalk.yellow("⚠️ No Git keys registered for this address."),
					);
					console.log(
						chalk.yellow(
							"   Register your Git SSH or PGP key first with: git-escrows register-key",
						),
					);
					console.log(
						chalk.yellow(
							"   Or use --no-verify-key to skip verification (not recommended)",
						),
					);
				} else {
					console.log(
						chalk.green(`✓ Git key registration verified - latest key found`),
					);
					console.log(
						chalk.gray(
							`  Key type: ${latestKeyClaim.keyType === 0 ? "PGP" : latestKeyClaim.keyType === 1 ? "SSH-Ed25519" : "SSH-Secp256k1"}`,
						),
					);
				}
			} catch (error) {
				console.log(
					chalk.yellow("⚠️ Could not verify Git key registration:", error),
				);
			}
		}
		...
		console.log(
			chalk.gray(`  Commit Algorithm: ${options.solutionAlgo || "sha1"}`),
		);
		console.log(
			chalk.gray(`  Additional Hosts: ${options.additionalHosts || "none"}`),
		);
		...
		try {
			...
			const escrowAttestation = await client.getEscrowAttestation({
				refUID: options.escrowUid as `0x${string}`,
			});
			...
			const decodedEscrow = decodeAbiParameters(
				escrowAbi,
			...
			console.log(
				chalk.yellow(
					"   The oracle may still detect the fulfillment, but arbitration request is recommended",
				),
			);
		}
		...
		console.log(
			chalk.yellow(
				"  1. The arbiter server will automatically test your solution",
			),
		);
		console.log(
			chalk.yellow("  2. If tests pass, you can collect the reward with:"),
		);
		console.log(
			chalk.yellow(
				`     git-escrows collect --escrow-uid ${options.escrowUid} --fulfillment-uid ${fulfillment.uid}`,
			),
		);
